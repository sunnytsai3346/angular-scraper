...
(async () => {
  ...
  for (const route of routesToScrape) {
    ...
    const content = await page.evaluate(() => {
      var all = document.querySelector("#info-display");
      console.log(all);
      // create range [0,1,2,....,all.length-1]
      // Array.from({length: all.length}, (v, k) => k).forEach((index) => {
      //   let element = all.item(index);
      //   console.log('index:',index,',element:',all.item(index));
      // })
      //return Array.from(document.querySelectorAll('body *' || 'body #info-display' || 'body #copyright-text'))
      return Array.from(document.querySelector("body #info-display"))
     // .map(el => el.innerText.trim())
     // .filter(text => text.length > 0 );
      //return Array.from(document.querySelectorAll('body *,#info-display,#copyright-text')).
      //  .map(el => el.innerText.trim())
      //  .filter(text => text.length > 0 );
    });
